...
async function generatePartsInvoicePDF(data: any): Promise<ArrayBuffer> {
  ...
  const compTextX = logoX + logoSize + 3;
  ...
  const addrLines = doc.splitTextToSize(data.showroom_address || 'N/A', splitX - compTextX - 3);
  let cy = y + 15;
  addrLines.slice(0, 2).forEach((ln: string) => { txt(ln, compTextX, cy, { size: 7, color: GRAY }); cy += 3.2; });
  txt(`Mobile ${data.showroom_phone || 'N/A'}`, compTextX, cy, { size: 7, color: GRAY });

  const metaRowH = compH / 2;
  vline(midX, y, y + compH);
  ...
  metaCell(midX,   y,            'Invoice Date:',   saleDate);
  metaCell(splitX, y + metaRowH, 'Payment Method:', data.payment_method || 'Cash');
  metaCell(midX,   y + metaRowH, 'Payment Status:', data.payment_status || '-');
  y += compH;

  // ── CUSTOMER ───────────────────────────────────────────────────────────────
  const custH = 20;
  box(L, y, R - L, custH);
  txt('Bill To:',                                  L + 3, y + 5,  { size: 8, bold: true });
  ...
  // ── ITEMS TABLE ────────────────────────────────────────────────────────────
  const itemBody = (data.items || []).map((it: any, i: number) => [
    String(i + 1),
    it.part_name || 'Part',
    it.part_code || '-',
    it.hsn_code  || '-',
  ...
  autoTable(doc, {
    ...
    head: [['#', 'Part Name', 'Part Code', 'HSN/SAC', 'Qty', 'Rate', 'Per', 'Amount']],
    body: itemBody,
    headStyles: {
  ...
  if (data.upi_id) {
    try {
      ...
      doc.addImage(qrDataUrl, 'PNG', (c2 + c3) / 2 - 13, y + 8, 26, 26);
    } catch {
  ...
  if (!signPlaced) {
    ...
    doc.circle(cx, y + 20, 12);
    doc.setLineWidth(0.3);
    doc.circle(cx, y + 20, 9.5);
    txt('SIGNATURE', cx, y + 21, { size: 7, bold: true, color: NAVY, align: 'center' });
    doc.setDrawColor(0, 0, 0);
    doc.setLineWidth(0.2);
  }
  ...
  if (data.notes || data.invoice_footer_note) {
    const noteH = 14;
    box(L, y, R - L, noteH);
    vline(splitX, y, y + noteH);
    if (data.notes) {
      txt('Notes:', L + 3, y + 4, { size: 7.5, bold: true });
      doc.splitTextToSize(data.notes, splitX - L - 6).slice(0, 2)
        .forEach((ln: string, i: number) => txt(ln, L + 3, y + 9 + i * 3.5, { size: 7, color: GRAY }));
    }
    if (data.invoice_footer_note) {
      txt('Terms:', splitX + 3, y + 4, { size: 7.5, bold: true });
      doc.splitTextToSize(data.invoice_footer_note, R - splitX - 6).slice(0, 2)
        .forEach((ln: string, i: number) => txt(ln, splitX + 3, y + 9 + i * 3.5, { size: 7, color: GRAY }));
    }
    y += noteH;
  }
...
export async function GET(
  ...
  try {
    ...
    // 1. Fetch the sale — NO customer join (no FK in schema cache)
    ...
    if (saleErr || !sale) {
      console.error('[parts-pdf] sale fetch failed:', saleErr?.message, 'id:', id);
    ...
    const flatItems = (rawItems || []).map((it: any) => {
      const part = partsMap[it.part_id] || {};
      return {
        part_name  : part.part_name   || 'Part',
        part_code  : part.part_code   || '-',
        hsn_code   : part.hsn_code    || '-',
        gst_pct    : part.gst_percentage || 0,
    ...
    // 4. Showroom / branding / billing — fetch in parallel
    const [{ data: showroomData }, { data: brandingData }, { data: billingData }, { data: addressData }] =
      await Promise.all([
    ...
    // 6. Store in Supabase Storage bucket (same bucket as service invoices)
    try {
      ...
      if (uploadErr) console.error('[parts-pdf] storage upload:', uploadErr.message);
    } catch (storageErr) {
      // Non-fatal — still return the PDF
      console.error('[parts-pdf] storage error:', storageErr);
    }
    ...
    console.error('[parts-pdf] unhandled error:', err);
